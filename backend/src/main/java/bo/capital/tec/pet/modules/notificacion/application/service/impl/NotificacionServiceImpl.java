package bo.capital.tec.pet.modules.notificacion.application.service.impl;
...
import bo.capital.tec.pet.modules.notificacion.application.service.NotificacionService;
...
public class NotificacionServiceImpl implements NotificacionService {
    ...
    public NotificacionResponseDTO getById(Long id) {
        Notificacion notificacion = notificacionRepository.selectById(id);
    ...
    public PagedResponse<NotificacionResponseDTO> getAll(int page, int size) {
        ...
        List<Notificacion> notificaciones = notificacionRepository.selectAll(offset, size);
        long total = notificacionRepository.countAll();
    ...
    public PagedResponse<NotificacionSummaryDTO> getByUsuarioId(Long usuarioId, boolean soloNoLeidas, int page, int size) {
        ...
        size = PaginationUtil.safeSize(size);
        int offset = page * size;
        List<Notificacion> notificaciones = notificacionRepository.selectByUsuarioId(usuarioId, offset, size);
        long total = notificacionRepository.countByUsuarioIdAndLeida(usuarioId, soloNoLeidas ? false : null);
    ...
    public void markAsRead(Long id) {
        ...
        notificacionRepository.updateLeida(id);
    }

    @Override
    @Transactional
    public void markAllAsRead(Long usuarioId) {
        List<Notificacion> notificaciones = notificacionRepository.selectByUsuarioId(usuarioId, 0, 10000);
        for (Notificacion notificacion : notificaciones) {
            if (Boolean.FALSE.equals(notificacion.getLeida())) {
                notificacionRepository.updateLeida(notificacion.getId());
            }
        }
    }
    ...
    public void delete(Long id) {
        ...
        notificacionRepository.deleteOld();
    }
