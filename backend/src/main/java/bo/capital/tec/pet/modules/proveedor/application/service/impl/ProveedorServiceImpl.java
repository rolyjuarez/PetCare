...
import bo.capital.tec.pet.modules.proveedor.application.service.ProveedorService;
...
import bo.capital.tec.pet.modules.proveedor.domain.port.ProveedorEspecialidadRepository;
...
public class ProveedorServiceImpl implements ProveedorService {
    ...
    private final ProveedorEspecialidadRepository proveedorEspecialidadRepository;
        ...
        if (dto.getServicioIds() != null) {
            for (Long servicioId : dto.getServicioIds()) {
                ProveedorEspecialidad pe = ProveedorEspecialidad.builder()
                        ...
                        .build();
                proveedorEspecialidadRepository.insert(pe);
            }
        }
    ...
    public ProveedorResponseDTO createFull(ProveedorFullCreateDTO dto) {
        ...
        if (dto.getEmail() != null && personaApi.selectByEmail(dto.getEmail()) != null) {
        ...
        Rol proveedorRol = rolApi.selectByNombre("PROVEEDOR");
        ...
        if (dto.getDisponibilidades() != null) {
            for (ProveedorFullCreateDTO.DisponibilidadItem item : dto.getDisponibilidades()) {
                Disponibilidad disp = Disponibilidad.builder()
                        ...
                        .activo(true)
                        .build();
                disponibilidadRepository.insert(disp);
            }
        }
    ...
    public ProveedorResponseDTO getById(Long id) {
        Proveedor proveedor = proveedorRepository.selectById(id);
        if (proveedor == null) {
    ...
    public PagedResponse<ProveedorSummaryDTO> getAll(String nombre, Long especialidadId, boolean activo, int page, int size) {
        ...
        List<Proveedor> proveedores = proveedorRepository.selectAll(nombre, null, offset, size);
        long total = proveedorRepository.countAll(nombre, null);
    ...
    public ProveedorResponseDTO update(Long id, ProveedorRequestDTO dto) {
        ...
        if (dto.getServicioIds() != null) {
            proveedorEspecialidadRepository.deleteByProveedorId(id);
            for (Long servicioId : dto.getServicioIds()) {
                ProveedorEspecialidad pe = ProveedorEspecialidad.builder()
                        .proveedorId(proveedor.getId())
                        .servicioId(servicioId)
    ...
    public ProveedorResponseDTO updateFull(Long id, ProveedorFullUpdateDTO dto) {
        ...
        if (!persona.getCi().equals(dto.getCi())) {
            Persona existing = personaApi.selectByCi(dto.getCi());
            if (existing != null) {
                throw new BusinessException("El CI ya esta registrado");
            }
        }
        if (dto.getEmail() != null && !dto.getEmail().equals(persona.getEmail())) {
            Persona existing = personaApi.selectByEmail(dto.getEmail());
            if (existing != null) {
                throw new BusinessException("El email ya esta registrado");
            }
        }
        ...
        if (persona.getDireccionId() != null) {
            Direccion direccion = direccionApi.selectById(persona.getDireccionId());
            if (direccion != null) {
        ...
        if (dto.getDisponibilidades() != null) {
            disponibilidadRepository.deleteByProveedorId(id);
            for (ProveedorFullCreateDTO.DisponibilidadItem item : dto.getDisponibilidades()) {
                Disponibilidad disp = Disponibilidad.builder()
                        .proveedorId(proveedor.getId())
                        .servicioId(item.getServicioId())
    ...
    public boolean hasReservas(Long proveedorId) {
        return reservaApi.countByProveedorId(proveedorId) > 0;
    }

    @Override
    @Transactional(readOnly = true)
    public List<ProveedorSummaryDTO> searchNearby(BigDecimal lat, BigDecimal lng, BigDecimal radioKm) {
        List<Proveedor> proveedores = proveedorRepository.searchNearby(lat, lng, radioKm);
    ...
    public List<ProveedorSummaryDTO> getByServicioId(Long servicioId) {
        List<Proveedor> proveedores = proveedorRepository.findByServicioId(servicioId);
        return proveedores.stream()
                .map(this::toSummaryDTO)
                .collect(Collectors.toList());
    }
    ...
    private ProveedorResponseDTO toResponseDTO(Proveedor proveedor) {
        ...
        if (persona != null) {
            ...
            if (direccionId != null) {
                var direccion = direccionApi.selectById(direccionId);
                if (direccion != null) {
        ...
        List<ProveedorEspecialidad> especialidades = proveedorEspecialidadRepository.selectByProveedorId(proveedor.getId());
        ...
        List<Disponibilidad> dispList = disponibilidadRepository.selectByProveedorYServicio(proveedor.getId(), null);
        List<ProveedorResponseDTO.DisponibilidadDTO> dispDTOs = new ArrayList<>();
        for (Disponibilidad d : dispList) {
            String servicioNombre = "";
            var svc = servicioApi.selectById(d.getServicioId());
