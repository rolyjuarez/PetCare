...
import bo.capital.tec.pet.modules.menu.dto.SubmenuResponseDTO;
...
import bo.capital.tec.pet.modules.menu.application.service.MenuService;
...
public class MenuServiceImpl implements MenuService {
    ...
    public MenuResponseDTO getById(Long id) {
        ...
        List<Submenu> submenus = submenuRepository.selectByMenuId(id);
    ...
    public PagedResponse<MenuSummaryDTO> getAll(String nombre, boolean activo, int page, int size) {
        ...
        List<Menu> menus = menuRepository.selectAll(offset, size);
        long total = menuRepository.countAll();
    ...
    public List<MenuResponseDTO> getMenusByRoleId(Long rolId) {
        List<Menu> menus = menuRepository.findByRolId(rolId);
        return menus.stream()
                .map(m -> {
                    List<Submenu> submenus = submenuRepository.selectByMenuId(m.getId());
    ...
    public List<MenuResponseDTO> getMenusByUsername(String username) {
        ...
        return menus.stream()
                .map(m -> {
                    List<Submenu> submenus = submenuRepository.selectByMenuIdAndUsername(m.getId(), username);
                    return toResponseDTO(m, submenus);
                })
                .collect(Collectors.toList());
    }
    ...
    public List<MenuResponseDTO> getMenuTree() {
        List<Menu> menus = menuRepository.selectAll(0, 1000);
        return menus.stream()
                .map(m -> {
                    List<Submenu> submenus = submenuRepository.selectByMenuId(m.getId());
                    return toResponseDTO(m, submenus);
