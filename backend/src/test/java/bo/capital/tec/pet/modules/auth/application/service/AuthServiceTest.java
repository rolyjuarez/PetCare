package bo.capital.tec.pet.modules.auth.application.service;

import bo.capital.tec.pet.modules.auth.dto.*;
import bo.capital.tec.pet.modules.auth.application.service.impl.AuthServiceImpl;
...
class AuthServiceTest {
    ...
    void login_ShouldReturnTokens_WhenCredentialsAreValid() {
        ...
        when(usuarioApi.findByUsername("admin")).thenReturn(usuario);
        ...
        when(usuarioApi.findRolesByUsuarioId(1L)).thenReturn(List.of("ADMIN"));
        when(usuarioApi.findPermissionsByUsuarioId(1L)).thenReturn(List.of("READ", "WRITE"));
        ...
        verify(usuarioApi).resetFailedAttempts(1L);
        verify(usuarioApi).updateLastAccess(1L);
        verify(usuarioApi).updateTokenRefresh(eq(1L), eq("refresh-token"));
    }
    ...
    void login_ShouldThrow_WhenUserNotFound() {
        ...
        when(usuarioApi.findByUsername("unknown")).thenReturn(null);
    ...
    void login_ShouldThrowAndIncrementAttempts_WhenWrongPassword() {
        ...
        assertThrows(BusinessException.class, () -> authService.login(request));
        verify(usuarioApi).incrementFailedAttempts(1L);
    }
    ...
    void login_ShouldBlockUser_WhenMaxAttemptsReached() {
        ...
        assertThrows(BusinessException.class, () -> authService.login(request));
        verify(usuarioApi).incrementFailedAttempts(1L);
        verify(usuarioApi).blockUser(1L);
    }
    ...
    void refresh_ShouldReturnNewTokens() {
        ...
        when(jwtTokenProvider.getExpirationMs()).thenReturn(3600000L);
        when(usuarioApi.findRolesByUsuarioId(1L)).thenReturn(List.of("ADMIN"));
        when(usuarioApi.findPermissionsByUsuarioId(1L)).thenReturn(List.of("READ"));
        ...
        verify(usuarioApi).updateTokenRefresh(eq(1L), eq("new-refresh-token"));
    }
    ...
    void logout_ShouldClearRefreshToken() {
        ...
        verify(usuarioApi).updateTokenRefresh(1L, null);
    }
