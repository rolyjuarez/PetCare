package bo.capital.tec.pet.modules.servicio.application.service;
...
import bo.capital.tec.pet.modules.servicio.application.service.impl.ServicioServiceImpl;
...
class ServicioServiceTest {
    ...
    void create_ShouldReturnServicioResponse() {
        ...
        when(servicioRepository.insert(any(Servicio.class))).thenReturn(1L);

        ServicioResponseDTO response = servicioService.create(request);

        assertNotNull(response);
        assertEquals("Peluqueria", response.getNombre());
        verify(servicioRepository).insert(any(Servicio.class));
    }

    @Test
    void getById_ShouldReturnServicio() {
        when(servicioRepository.selectById(1L)).thenReturn(servicio);
    ...
    void getById_ShouldThrow_WhenNotFound() {
        when(servicioRepository.selectById(99L)).thenReturn(null);
    ...
    void getAll_ShouldReturnPagedResponse() {
        when(servicioRepository.selectAll(any(), any(), anyInt(), anyInt()))
                .thenReturn(List.of(servicio));
        when(servicioRepository.countAll(any(), any())).thenReturn(1L);
    ...
    void getActive_ShouldReturnActiveServicios() {
        when(servicioRepository.selectActive()).thenReturn(List.of(servicio));
    ...
    void update_ShouldReturnUpdatedServicio() {
        ...
        when(servicioRepository.selectById(1L)).thenReturn(servicio).thenReturn(updatedServicio);

        ServicioResponseDTO response = servicioService.update(1L, request);

        assertNotNull(response);
        assertEquals("Peluqueria Premium", response.getNombre());
        verify(servicioRepository).update(any(Servicio.class));
    }
    ...
    void delete_ShouldCallSoftDelete() {
        ...
        verify(servicioRepository).softDelete(1L);
    }
